...
#define HIGH_WM (FIFO_SIZE)       // start draining when full
#define LOW_WM  (FIFO_SIZE / 2)   // stop draining when half full
// ===============================
...
static inline void update_drain_req_locked()
{
  // Call ONLY while holding fifoMux
  static bool draining = false;

  if (!draining && fifo_count >= HIGH_WM) draining = true;
  if ( draining && fifo_count <= LOW_WM)  draining = false;

  digitalWrite(DRAIN_REQ_PIN, draining ? HIGH : LOW);
}
...
static inline bool fifo_push(uint8_t b)
{
  ...
  if (fifo_count < FIFO_SIZE) {
    ...
    // Option 1: drop newest (current behavior)
    // ok stays false
    //
    // Option 2 (often better): overwrite oldest:
    // fifo[fifo_head] = b;
    // fifo_head = (fifo_head + 1) % FIFO_SIZE;
    // fifo_tail = (fifo_tail + 1) % FIFO_SIZE; // drop oldest
    // ok = true;
  }

  update_drain_req_locked();
